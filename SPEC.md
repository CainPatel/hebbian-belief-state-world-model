...
**Environment.** Custom gridworld POMDP, fully controlled: 7×7 to 11×11 grid, 1 to 4 objects (keys, doors, movable markers), agent observes a 3×3 window. Discrete token observations. Scripted + random exploration policies generate trajectories; no RL needed.

**Model.** BDH-GPU, 1 to 5M params, trained on next-token prediction over `(action, observation)` sequences. Single consumer GPU / free Colab scale.
...
2. Gridworld env + tokenizer + trained next-token model (2 to 3 weeks part-time).
3. Study 1 probes and writeup (3 to 4 weeks part-time). ← first real deliverable
4. Study 2, then optionally Study 3.

**Compute budget.** Studies 1 to 2: single consumer GPU or free Colab. Study 3: one mid-tier GPU (or a few hundred dollars of cloud).
...
- **Research-artifact codebase**: BDH's repo is nanoGPT-style, not a library; the graph picture and the GPU tensor implementation don't map one-to-one. Budget real time for reading Sections 2 to 3 of the paper against the code before writing anything.
