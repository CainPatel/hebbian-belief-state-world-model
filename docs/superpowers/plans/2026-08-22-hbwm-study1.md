...
**Goal:** Build the Study 1 pipeline (an explicit-σ BDH core, a gridworld POMDP, LSTM/RWKV baselines, σ instrumentation, linear probes) and run the preregistered H1 to H4 evaluation, producing `RESULTS.md`.

**Architecture:** Upstream BDH is vendored untouched; `HBWMCore` subclasses it with a γ-decay parallel forward (training) and a mathematically equivalent recurrent `step()` that materialises σ (N×D per head per level) for inference, instrumentation and later Study 2. A numpy gridworld emits `(action, observation)` token sequences; one trainer trains BDH/LSTM/RWKV; recorders drive `step()` to extract per-step state features; linear probes + preregistered decision rules evaluate H1 to H4.
...
| `hbwm/matrix.py` | experiment matrix E1 to E3 + probes + evaluate orchestration (+ CLI) |
...
The plan has five phases matching spec milestones M1 to M5. Each phase ends with working, tested software; Phase 5 is the experiment run.
...
- [ ] **Step 4: Run tests** → `uv run pytest tests/test_probes_run.py -v` → 1 passed (≈ 10 to 30 s on CPU).
...
These tasks run the experiment matrix. They are long-running (hours), so each "run" step launches the command in the background with a log file and a later step checks the artefacts. Do **not** start any E1 to E3 run before the preregistration commit in Task 30 exists.
...
```
Expected per BDH checkpoint: small-feature stage (18 specs, three recorder passes) + σ_full stage (≤ 3 levels, 25 GB fp16 cache each, one at a time) + atlas: roughly 1 to 2 h on MPS; baselines minutes. Disk: ≤ 75 GB transient under `runs/…/probes/cache/` (deleted on completion).
...
- [ ] `RESULTS.md` states H1 to H4 decisions via the preregistered rules, plus the prediction-quality table
