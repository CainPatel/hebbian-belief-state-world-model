...
- Produces: `derot_feature_stats(X, positions, shape: StateShape, freqs, chunk=256) -> tuple[np.ndarray[F] float32, np.ndarray[F] float32]`; `train_family_probes(X, y, n_classes, shape, spec, l2_grid, positions=None, epochs=20, lr=1e-3, batch=512, seed=0, device="cpu", freqs=None, n_input=None) -> dict[tuple[float, int], StructuredProbe]` keyed by `(l2, restart_index)`; and in `hbwm/probes/probe.py`, `predict_proba_stream(probes, it, n, n_classes, device="cpu", columns=None, positions=None)`.
...
Expected: 7 passed in the new file, and every existing `tests/test_probe.py` test still passes (the `positions=None` default is the Study 1 path).
...
FAMILIES_BDH = {
    ...
    "mlp_state_L3": (0.15, 0.16, None, False),
    "mlp_rownorm_L3": (0.19, 0.20, None, False),  # BDH-only reduction: context, feeds H7
}
FAMILIES_BASE = {
...
Expected: 3 passed in the new file; every existing `tests/test_evaluate.py` test still passes.
