...
- **Held verbatim from Study 1** (spec section 4.7), so cross-study comparison stays legitimate: per-feature standardization (`feature_stats`, mean and std from `probe_train`, std below 1e-6 mapped to 1.0), Adam lr 1e-3, 20 epochs, batch 512, L2 grid `[1e-4, 1e-3, 1e-2, 1e-1]` selected on `probe_val`, 95% bootstrap CI resampled over test episodes (`bootstrap_ci`, `n_boot=1000`), the six steps-since-seen buckets `["1-4", "5-8", "9-16", "17-32", "33-64", "65+"]`, and the eligibility rule `steps_since_seen >= 1 and not visible and not stale`.
...
- **Two spec deviations are forced by cost and are resolved by an explicit spec amendment in Task 3, before the Task 13 preregistration commit.** They are listed in Task 3 and must be confirmed with the controller when that task is reached. No other deviation from the spec is permitted after Task 13.

## File Structure

| path | responsibility |
|---|---|
| `hbwm/probes/structured.py` | `StateShape`, `state_shape`, the five probe-family modules, `derotate`, sparse random projection, `build_family`, `family_specs`, `param_count`, `train_family_probes` |
...
| `tests/test_derotation.py` | derotation invertibility, phase algebra, non-equivalence to `flat_linear` |
...
**Interfaces:**
- Produces: `StateShape(n_heads: int, rows: int, cols: int, rotary: bool = False)` with properties `n_features -> int`, `saturation_rank -> int`, and methods `rank_fraction(r: int) -> float`, `shared_saturation_rank -> int`; `state_shape(model, feature: str = "sigma_full") -> StateShape`; `param_count(family: str, shape: StateShape, n_classes: int, rank: int | None = None) -> int`; module constants `STUDY2_RANKS = (1, 4, 16)`, `STUDY2_RESTARTS = 3`, `MLP_HIDDEN = 512`, `RANDPROJ_DIM = 4096`, `RANDPROJ_DENSITY = 64`.
...
def param_count(family: str, shape: StateShape, n_classes: int, rank: int | None = None) -> int:
...
### Task 3: The capacity-control families, and the two forced spec amendments
...
- Modify: `docs/superpowers/specs/2026-08-27-hbwm-study2-associative-readout-design.md`
...
**Two spec amendments this task must make first.** Both are cost-forced, both are discoveries from writing executable code, and both must be confirmed with the controller before Task 13's preregistration commit freezes the spec.

1. **`mlp_randproj` uses a sparse random projection, not a dense Gaussian one.** Spec 4.5 says "a fixed random Gaussian projection of the standardized flat $\sigma$ to 4096 dimensions". A dense projection costs `24000 * 524288 * 4096` = 5.2e13 multiply-adds per checkpoint-level, roughly 3 h on this machine and about 11 h across the four caches, which would nearly double the whole study's compute for a control. A very sparse random projection with 64 nonzeros per output dimension costs `24000 * 4096 * 64` = 6.3e9, which is seconds, and preserves the Johnson-Lindenstrauss property the control relies on. Amend the sentence to "a fixed sparse random projection (64 signed nonzeros per output dimension, seeded per checkpoint, seed and indices recorded)".
2. **On a baseline state, family 5 has a single matched arm, `mlp_state`.** Spec 4.5 calls both MLP families reshape-invariant, which is true of `mlp_randproj` but not of `mlp_rownorm`: row norms of the LSTM reshape `[4, 350]` would give 4 features, and projecting a 1400-dimensional state up to 4096 is an expansion, not a reduction. Both reductions exist only because 524,288 is too wide to feed an MLP directly, and a baseline state has nothing to reduce. Amend spec 4.5 and 5.1 to say: on a baseline, family 5's matched arm is `mlp_state`, an MLP on the standardized full state vector with the same hidden width 512, and both BDH members are compared against it. Note the resulting asymmetry (BDH has two members of family 5, each baseline has one) in the results table.

- [ ] **Step 1: Amend the spec**

In `docs/superpowers/specs/2026-08-27-hbwm-study2-associative-readout-design.md`, section 4.5, replace the `mlp_randproj` bullet with:
```markdown
- `mlp_randproj`: a fixed sparse random projection of the standardized flat $\sigma$ to 4096 dimensions (64 signed nonzeros per output dimension, seeded per checkpoint, seed recorded), then $4096 \to 512 \to C$. A dense Gaussian projection at this width would cost about 5.2e13 multiply-adds per checkpoint-level and roughly double the study's compute for a control, while a very sparse projection preserves the Johnson-Lindenstrauss property at 6.3e9.
```
and replace the sentence beginning "Both are reshape-invariant" with:
```markdown
On a baseline state, family 5 has a single matched arm, `mlp_state`: an MLP on the standardized full state vector at the same hidden width 512. Both reductions above exist only because 524,288 is too wide to feed an MLP directly, and a 1,400- or 3,520-dimensional state has nothing to reduce, so `mlp_state` is the correct matched definition and both BDH members are compared against it. Family 5 therefore has two members on BDH and one on each baseline, and the results table says so.
```
Then verify the no-dash constraint still holds. The check is written with unicode escapes so this plan
file does not itself contain the characters it forbids:
```bash
uv run python -c "import re,sys;bad=[(f,i) for f in sys.argv[1:] for i,l in enumerate(open(f),1) if re.search(r'[\u2014\u2013]',l)];print(bad or 'clean');sys.exit(1 if bad else 0)" \
  docs/superpowers/specs/2026-08-27-hbwm-study2-associative-readout-design.md
```
...
class MLPProbe(StructuredProbe):
    ...
    Used for `mlp_rownorm` (input = sigma_rownorm, 8192 dims), `mlp_randproj` (input = the sparse
    random projection of flat sigma, 4096 dims) and, on the baselines, `mlp_state` (input = the full
    state vector). The reduction happens outside the module, so this is one class for all three.
    """
...
Amends spec 4.5: sparse (not dense Gaussian) random projection, and mlp_state as
family 5's single matched arm on the baselines. Both are cost-forced and both
precede the preregistration commit."
```
...
- Produces: `derotate(sigma: Tensor[..., nh, P, Q], t: Tensor[B] | float, freqs: Tensor[P]) -> Tensor` (applies $R(-t)$ along the row axis); `DerotProbe(inner: StructuredProbe, shape: StateShape, freqs: Tensor[P], mean=None, std=None)` whose `forward(x, t=None)` **standardizes, then derotates, then delegates** to an identity-standardized inner, and whose `set_positions` stores the batch's absolute token positions.

**Standardization order matters and is fixed here.** The derotation mixes pairs of rows, so standardizing before it and standardizing after it are different probes. This plan puts the derotation **after** standardization: the probe's input space is the standardized state, exactly as for every other family, and the derotation is part of the readout. The alternative (fit `feature_stats` on derotated rows) would need a second full pass over the 25 GB cache per family for no gain. `DerotProbe` therefore owns the mean and std buffers, and `build_family` constructs its inner probe with no standardization.
...
class DerotProbe(StructuredProbe):
    ...
    def forward(self, x, t=None):
        ...
        s = self.shape
        # Standardize FIRST, then derotate: the inner probe is built with identity mean/std, so the
        # transform order is the same when fitting and when streaming.
        z = self.standardize(x).view(-1, s.n_heads, s.rows, s.cols)
        return self.inner(derotate(z, pos, self.freqs).reshape(x.shape[0], -1))
```
...
`family_specs` returns, for a rotary state (BDH): `flat_linear`, `query_rank_r` at r in (1, 4, 16), `shared_query_rank_r` at r in (1, 4, 16), `derot_flat_linear`, `derot_query_rank_r` at r in (1, 4, 16), `mlp_rownorm`, `mlp_randproj`. That is 13 specs. For a non-rotary state (a baseline): `flat_linear`, `query_rank_r` x 3, `shared_query_rank_r` x 3, `mlp_state`. That is 8 specs, because the two `derot_*` families are by definition their undecorated counterparts (spec 5.1) and are recorded as aliases rather than refitted.
...
def test_restarts_are_three_for_factorized_families_and_one_otherwise():
    ...
    assert got["flat_linear"] == 1 and got["mlp_rownorm"] == 1 and got["derot_flat_linear"] == 1
...
def family_specs(shape: StateShape) -> list[FamilySpec]:
    ...
    if shape.rotary:
        ...
        out.append(FamilySpec("mlp_state", input_kind="state"))
    return out
...
**Interfaces:**
- Consumes: `build_family`, `FamilySpec`, `feature_stats` (from `hbwm.probes.probe`, unchanged).
- Produces: `train_family_probes(X, y, n_classes, shape, spec, l2_grid, positions=None, epochs=20, lr=1e-3, batch=512, seed=0, device="cpu", freqs=None, n_input=None) -> dict[tuple[float, int], StructuredProbe]` keyed by `(l2, restart_index)`; and in `hbwm/probes/probe.py`, `predict_proba_stream(probes, it, n, n_classes, device="cpu", columns=None, positions=None)`.
...
from hbwm.probes.probe import accuracy, predict_proba
from hbwm.probes.structured import FamilySpec, StateShape, train_family_probes
...
Expected: 5 passed in the new file, and every existing `tests/test_probe.py` test still passes (the `positions=None` default is the Study 1 path).
...
def _study2_level(model, shape, freqs, data, pairs, specs, cfg, n_classes, device, out_dir, cache_dir,
    ...
    for spec in specs:
        ...
        results[spec_label(spec) + tag + suffix] = {
            ...
            "n_params": (None if spec.name.startswith("mlp_")
                         else param_count(spec.name.replace("derot_", ""), shape, n_classes, spec.rank)),
    ...
    if not suffix:  # H8 readout (spec 7) on this level's best-on-val spec
        ...
        if len(h3p):
            ...
            np.savez(out_dir / f"h8{tag}.npz", p_old=probs[rows, h3p.old_cell],
                     p_new=probs[rows, h3p.new_cell], ep=h3p.ep, t=h3p.t,
                     steps_since_reobs=h3p.steps_since_reobs, visible_now=h3p.visible_now)
...
Expected: 3 passed in the new file; every existing `tests/test_probes_run.py` test still passes.
...
from hbwm.probes.decisions import h5_decision, h6_decision, h7_attribution, h8_latency
...
Expected: 9 passed in the new file; every existing `tests/test_decisions.py` test still passes.

- [ ] **Step 5: Commit**

```bash
git add hbwm/probes/decisions.py tests/test_decisions_study2.py
git commit -m "feat(probes): H5 to H8 preregistered decision functions"
```
...
FAMILIES_BDH = {
    "flat_linear_L3": (0.10, 0.10, None, False),
    "query_rank_4_L3": (0.28, 0.29, 4, False),
    "shared_query_rank_4_L3": (0.20, 0.21, 4, False),
    "mlp_rownorm_L3": (0.19, 0.20, None, False),
}
FAMILIES_BASE = {
    "flat_linear": (0.17, 0.17, None, False),
    "query_rank_4": (0.18, 0.18, 4, True),
    "shared_query_rank_4": (0.16, 0.16, 4, True),
    "mlp_state": (0.17, 0.18, None, False),
}


def _write(run_dir, families, shape):
    (run_dir / "probes2").mkdir(parents=True, exist_ok=True)
    for label, (test_acc, val_acc, rank, sat) in families.items():
        (run_dir / "probes2" / f"{label}.json").write_text(json.dumps({
            "family": label.split("_L")[0], "rank": rank, "level": 3 if "_L" in label else None,
            "test_acc": test_acc, "val_acc": {"0.001/0": val_acc}, "best_l2": 0.001, "best_restart": 0,
            ...
            "saturated": sat, "n_params": 1, "input_kind": "flat", "n_restarts": 1,
...
def aggregate_study2(root, exp="study2", src_exp="study1", data_dir=None) -> dict:
    ...
    matched = [k for k in bdh if all(_alias_key(k, table[m]) for m in ("lstm", "rwkv"))]
    ...
    mlp_keys = [k for k in bdh if k.startswith("mlp_")]
...
def _alias_key(key, other):
    ...
    Two aliases exist: a derot family on a baseline IS its undecorated counterpart (nothing to undo),
    and both BDH members of family 5 are matched against the baseline's single `mlp_state` arm.
    """
    if key in other:
        return key
    if key.startswith("derot_") and key[len("derot_"):] in other:
        return key[len("derot_"):]
    if key.startswith("mlp_") and "mlp_state" in other:
        return "mlp_state"
    return None
...
def write_outputs_study2(agg: dict, out_dir) -> None:
    ...
    lines = ["## Study 2 probe accuracy (test, best level and hyperparameter per seed; mean over 3 seeds)",
             "", "| model | family | rank | eff. rank frac | saturated | acc | #params | n_train |",
             "|---|---|---|---|---|---|---|---|"]
...
Expected: 2 passed in the new file; every existing `tests/test_evaluate.py` test still passes.
...
Probe families, all on identical eligible pairs, splits, standardization, L2 grid, bootstrap CI and
steps-since-seen buckets, and all trained on the same 24,000-pair stratified subsample: `flat_linear`
(control), `query_rank_r`, `shared_query_rank_r`, `derot_flat_linear`, `derot_query_rank_r` with
r in {1, 4, 16} chosen on probe_val, and the capacity controls `mlp_rownorm` and `mlp_randproj`. Every
family that can be defined on a baseline state is run on the LSTM state (1,400, reshaped 4 x 350) and
the RWKV state (3,520, reshaped 20 x 176); derotation on a baseline is the identity, and family 5's
matched baseline arm is `mlp_state`. Every factorized number is reported with its effective rank
fraction r / min(P, Q), clipped at 1.00, and 1.00 marks a saturated arm.
...
PY
```
Expected: 9 lines; 13 specs per BDH seed for a single-level seed and 26 for seed 0 (two levels), 8 specs per baseline seed; no surviving cache directories.
...
**Study 2 is done** when this task's commit exists: every checkpoint-level in spec section 6 complete, H5 to H8 decided by the rules frozen at the Task 13 preregistration commit, parameter counts, `n_train` and effective rank fractions reported per family, H8 coverage reported, and the exploratory measurements reported under their own heading. Positive or negative.
