...
| 7 | Number of preregistered rules | Exactly four hypotheses (H5 to H8), plus one eligibility criterion attached to H6 (degeneracy, section 7) | The whole point of a pivot is that it must not become a fishing expedition. The degeneracy criterion is not a fifth hypothesis: it decides nothing on its own, it only says which arms are allowed to decide H6 |
...
**Preregistered reporting requirement.** Every arm in the results tables carries its **training accuracy** alongside its validation and test accuracy, and its **parameter count**, so a reader can audit the degeneracy call of section 7 rather than take it on trust. `mlp_state` reports its first-layer parameter count specifically (524,288 x 512 on BDH), because 268.5 M parameters against 24,000 training examples is the single number that tells a reader how to read that row. Every factorized number additionally carries its effective rank fraction $r / \min(P, Q)$, clipped at 1.00, and a fraction of 1.00 marks the arm as saturated. `shared_query_rank_r` uses the same $\min(P, Q)$ denominator so that the two factorized families are read on one scale; its own saturation point is the $P$ column above, which coincides with $\min(P, Q)$ for both baselines and exceeds it only for BDH, where the grid tops out at $r = 16$ and no arm saturates either way. Saturated arms are still fitted and still reported; they are simply labeled for what they are.
...
- **Degeneracy criterion (preregistered, applies to H6's selection).** An arm is declared **degenerate** and is excluded from H6's best-matched-family selection if its `probe_train` accuracy exceeds 0.95 while its `probe_val` accuracy is below twice the majority-class chance rate, at every value of the L2 grid. Where a family has restarts, each L2 value is represented by the restart that maximizes validation accuracy, which is the one selection would pick; the criterion is applied to the seed-mean training and validation accuracies so it yields one deterministic verdict per arm. Degenerate arms are still fitted, still reported with their parameter count, training accuracy and validation accuracy, and are labeled degenerate in the results table. **The criterion applies to every arm of every family and every model, not only to `mlp_state`.** Its purpose is to stop a probe that has merely memorized its training set from deciding a hypothesis; it is objective, symmetric across models, and fixed before any run.

  This is needed because the comparison carries a dimensionality asymmetry that is not specific to any one family: BDH's state is 524,288-dimensional against 1,400 and 3,520 for the baselines, in Study 2 as it was in Study 1. That is a property of the comparison to disclose, not a reason to drop a family, and the criterion is the mechanical disclosure.
...
| degeneracy criterion | a synthetic degenerate arm (train accuracy above 0.95 and validation accuracy below twice chance at every L2) is flagged and excluded from H6's selection; a synthetic healthy arm is not; an arm that is degenerate at some L2 values but not all is not; the flag is computed from the seed-mean accuracies and applies identically to every family and model |
...
| M5 | README Study 2 preregistration commit, then feature caches and probe fits for all 9 checkpoints, bridge rows, aggregation, tables, figures, write-up | preregistration commit exists and precedes every Study 2 run; every checkpoint-level in section 6 complete; H5 to H8 decided by the stated rules; parameter counts, `n_train`, training accuracy, effective rank fraction and the degeneracy verdict reported per arm; H8 coverage reported; section 4.8 measurements reported under an exploratory heading |
...
| The study becomes a fishing expedition | Exactly four preregistered hypotheses plus one H6 eligibility criterion; families, rank grid and degeneracy thresholds fixed before implementation; everything else labeled exploratory and unable to decide anything |
| `mlp_state` on BDH has 268.5 M first-layer parameters against 24,000 training examples and may produce a number nobody can interpret, and an uninterpretable arm that wins or loses H6 would be worse than no arm | The preregistered degeneracy criterion of section 7 excludes any memorizing arm from H6's selection mechanically, symmetrically across models, and by a rule fixed before any run. The arm is still fitted and still reported with its parameter count, training accuracy and validation accuracy, so the call is auditable. Dropping the family instead would leave family 1 as H6's only genuinely matched family, which is thinner coverage than the headline deserves |
