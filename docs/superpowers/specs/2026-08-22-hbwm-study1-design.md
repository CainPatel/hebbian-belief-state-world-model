...
**Scope of this document:** Study 1 of [SPEC.md](../../../SPEC.md) (§5, "Does σ encode readable beliefs?") — i.e. SPEC.md milestones 1–3 — plus the minimal interface stub that lets Study 2 reuse the core unchanged.
...
| 2 | Primary compute | Local Apple M5 Max / 128 GB, **MPS**, fp32, `torch.compile` off by default | Models are 1–5M params; trains locally in ≤ ~1 h/run. CUDA must work but is untested-by-default |
...
**Labels and eligibility.** Target for (episode, t, object k) = the object's true cell id ∈ {0 … G²−1} at step t. **Eligible** iff `steps_since_seen ≥ 1` (seen before, out of view now) **and not `stale`**. Per episode and object: up to 8 eligible pairs sampled stratified over `steps_since_seen` buckets {1–4, 5–8, 9–16, 17–32, 33–64, 65+}.
...
- **H2** — accuracy of the best `sigma_full` probe by `steps_since_seen` bucket. "Graceful" iff acc(33–64) ≥ 0.5 · acc(1–4) **and** no bucket is < 50 % of its predecessor. Reported for γ ∈ {1.0, 0.99, 0.97} and for both baselines.
...
Estimate on MPS: BDH ≈ 0.5–1 h/run; LSTM/RWKV similar; feature extraction ≈ minutes per checkpoint; `sigma_full` probes ≈ 10–20 min each. Whole matrix ≈ 1–2 days of background wall-clock.

**Preregistration mechanics.** `README.md` contains §4.5's hypotheses and decision rules verbatim and is committed **before** any E1–E3 run starts; `RESULTS.md` records that commit hash. Any post-hoc analysis is labelled as such.
...
| area | tests |
|---|---|
| core | equivalence contract (i)–(iv) of §4.1; decay-mask values; `share_layers=False` allocates distinct parameters; `synapse()` equals explicit `σ @ encoder_v` on a tiny config |
...
| probes | oracle features (one-hot last-seen cell) → 100 %; shuffled labels → chance; eligibility mask matches a hand-computed example; top-k accuracy is non-decreasing in k up to noise (asserted on synthetic data); H1–H4 decision functions on hand-built inputs |
...
| M4 | recorder, feature extractors, atlas, `belief()`, probes, H1–H4 evaluators, notebooks skeleton | probe tests + smoke test green; `evaluate` produces tables from a tiny run |
| M5 | README preregistration commit → E1–E3 → probes → `RESULTS.md`, figures, heatmap notebook | every run in §5 complete; H1–H4 decided by the stated rules; perplexity table; notebook renders |
